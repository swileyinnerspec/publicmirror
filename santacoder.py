...
if len(sys.argv) == 2 and sys.argv[1] == "serve":
    ...
    flantokenizer = T5Tokenizer.from_pretrained("google/flan-t5-xl")
    flanmodel = T5ForConditionalGeneration.from_pretrained("google/flan-t5-xl").to("cpu")
    def flangen(input_text):
        if(len(input_text)>1):
            input_ids = flantokenizer(input_text, return_tensors="pt").input_ids.to("cpu")
            output = flanmodel.generate(input_ids, max_length=100)
